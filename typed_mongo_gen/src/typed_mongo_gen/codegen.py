...
def _annotation_to_source(
    ...
    if isinstance(annotation, typing.TypeAliasType):
        return _annotation_to_source(
            annotation.__value__, module_aliases, model_dict_names
        )
    ...
    if origin is typing.Annotated:
        return _annotation_to_source(
            get_args(annotation)[0], module_aliases, model_dict_names
        )

    # Union / Optional (X | Y)
    if origin is types.UnionType or origin is typing.Union:  # pyright: ignore[reportDeprecated]
        parts = [
            _annotation_to_source(a, module_aliases, model_dict_names)
...
def _query_value_type_src(
    ...
    if origin is types.UnionType or origin is typing.Union:  # pyright: ignore[reportDeprecated]
        parts = [
            _query_value_type_src(a, module_aliases, model_dict_names)
            for a in get_args(annotation)
        ]
        return " | ".join(parts)
    if origin is list:
        ...
        if args:
            ...
            full_list_src = _annotation_to_source(
                annotation, module_aliases, model_dict_names
            )
...
def _write_nested_dict(
    ...
    write_typeddict(stub_f, dict_name, entries)
...
def _write_model(
    ...
    model_entries = [
        (
            path,
            _annotation_to_source(path_types[path], module_aliases, model_dict_names),
        )
    ...
    write_typeddict(stub_f, f"{model_name}Query", query_entries, total=False)
    ...
    write_typeddict(stub_f, f"{model_name}Fields", fields_entries, total=False)
    ...
    if has_numeric:
        ...
        stub_f.write(
            f"type {model_name}NumericFields = dict[Literal[{numeric_keys}], int | float]\n\n"
        )

    # ArrayPath: Literal of all list-typed field paths
    array_path_list = sorted(
        p for p, t in path_types.items() if extract_list_element_type(t) is not None
    )
    ...
    if has_arrays:
        ...
        stub_f.write(
            f"type {model_name}ArrayElementFields = dict[{model_name}ArrayPath, Any]\n"
        )
        stub_f.write(
            f"type {model_name}ArrayPushFields = dict[{model_name}ArrayPath, Any]\n"
        )
        stub_f.write(
            f"type {model_name}ArrayPopFields = dict[{model_name}ArrayPath, Literal[1, -1]]\n\n"
        )
    ...
    stub_f.write(
        f'type {model_name}UnsetFields = dict[{model_name}OptionalPath, Literal[""]]\n\n'
    )
    ...
    stub_f.write(
        f"type {model_name}PipelineSetFields = dict[{model_name}Path, Any]\n\n"
    )
    ...
    write_typeddict(stub_f, f"{model_name}Update", update_entries, total=False)

    # --- Safe aggregation stage TypedDicts ---

    write_typeddict(
        stub_f, f"{model_name}MatchStage", [("$match", f"{model_name}Query")]
    )
    write_typeddict(
        stub_f,
        f"{model_name}SortStage",
        [("$sort", f"dict[{model_name}Path, Literal[1, -1]]")],
    )
    write_typeddict(stub_f, f"{model_name}LimitStage", [("$limit", "int")])
    write_typeddict(stub_f, f"{model_name}SkipStage", [("$skip", "int")])
    write_typeddict(
        stub_f, f"{model_name}SetStage", [("$set", f"{model_name}PipelineSetFields")]
    )
    write_typeddict(
        stub_f, f"{model_name}AddFieldsStage", [("$addFields", "dict[str, Any]")]
    )
    if optional_paths:
        write_typeddict(
            stub_f,
            f"{model_name}AggUnsetStage",
            [("$unset", f"{model_name}OptionalPath | list[{model_name}OptionalPath]")],
        )
    ...
    write_typeddict(
        stub_f,
        f"{model_name}GroupFields",
        [
            (
                "_id",
                f"{model_name}RefPath | list[{model_name}RefPath] | dict[str, {model_name}RefPath] | None",
            ),
        ],
    )
    write_typeddict(
    ...
    func_name = _to_snake_case(model_name)
    stub_f.write(
        f"def {func_name}_aggregation_step(step: {model_name}UnsafeStage) -> AggregationStep: ...\n\n"
    )
...
def _collect_all_nested_models(
    ...
    while to_visit:
        ...
        for field_name in model.model_fields:
            annotation = model.model_fields[field_name].annotation
            for nested in extract_base_models(annotation):
